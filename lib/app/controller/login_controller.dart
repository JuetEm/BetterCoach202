...
class LoginController {
  ...
  Future<FA.User?> googleSignIn() async {
    ...
    print("google log in : currentUser email : ${currentUser.email}, currentUser photoURL : ${currentUser.photoURL}, currentUser displayName : ${currentUser.displayName}");
  ...
  Future<FA.User?> kakaoSignIn() async {
    ...
    print("user?.kakaoAccount?.profile?.profileImageUrl : ${user?.kakaoAccount?.profile?.profileImageUrl}");
    ...
    final customToken = await _firebaseauthRemoteDataResource.createCustomToken({
    'uid':id,
    'displayName':name,
    'email':email,
    'photoURL':photoURL ?? '',
    ...
    final FA.UserCredential userCredential = await FA.FirebaseAuth.instance.signInWithCustomToken(customToken);

    final FA.User? fUser = userCredential.user;

    currentUser = _firebaseAuth.currentUser!;
    print("kakao login : currentUser email : ${currentUser.email}, currentUser photoURL : ${currentUser.photoURL}, currentUser displayName : ${currentUser.displayName}");

    String? fEmail = fUser?.email;
    String? fUrl = fUser?.photoURL;
    String? fName = fUser?.displayName;

    print("kakao login : fUser email : ${fEmail}, fUser url : ${fUrl}, fUser name : ${fName}");
